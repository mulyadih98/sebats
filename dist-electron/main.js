...
function requireLib() {
  ...
  lib$3.exports = profiles;
  lib$3.exports.proxyBundle = proxyBundle;
  lib$3.exports.default = profiles;
...
lib$1.createLRU = createLRU$3;
const { createLRU: createLRU$2 } = lib$1;
...
const { createLRU: createLRU$1 } = lib$1;
...
const EventEmitter$3 = require$$0$5.EventEmitter;
...
let Connection$3 = class Connection extends BaseConnection$2 {
  promise(promiseImpl) {
...
function createConnection(opts) {
  return new Connection$2({ config: new ConnectionConfig$1(opts) });
}
...
var pool$3 = BasePool$1;
var pool$2;
var hasRequiredPool;
function requirePool() {
  if (hasRequiredPool) return pool$2;
  hasRequiredPool = 1;
  const EventEmitter2 = require$$0$5.EventEmitter;
  const makeDoneCb = requireMake_done_cb();
  const PromisePoolConnection = requirePool_connection();
  const inheritEvents = requireInherit_events();
  const BasePool3 = pool$3;
...
const Connection2 = pool.promise();
const testConnection = async () => {
  try {
    const [rows] = await Connection2.query("SELECT NOW() AS currentTime");
    const row = rows;
    console.log("Koneksi pool berhasil. Waktu server:", row[0].currentTime);
  } catch (error) {
    console.log("Gagal konek via pool:", error.message);
  }
};
const setupIPC = () => {
  ipcMain.handle("get-products", async () => {
    try {
      const [rows] = await Connection2.query("SELECT * FROM products");
      return rows;
    } catch (error) {
